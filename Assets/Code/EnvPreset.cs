...
public class EnvPreset : MonoBehaviour {
	...
	void OnEnable() {
		...
		foreach(var vs in Object.FindObjectsOfType<VegetationSystem>()) {
			vs.indirectColorScale = Mathf.Clamp(growthIndirectScale, 0f, 10f);
			vs.indirectProbeDirection = Mathf.Clamp01(growthIndirectDirection);
			vs.directOcclusionBoost = growthDirectOcclusionBoost;
			vs.UpdateMaterials();
			vs.TickIndirectLighting(true);
			vs.ForceUpdateIngameIndirect();
		}
	}
}
